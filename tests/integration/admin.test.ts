...
async function auditCount(action: string, entityId: string): Promise<number> {
  const r = await asService(
    (tx: Sql) =>
      tx`select count(*)::int as n from public.audit_events where action = ${action} and entity_id = ${entityId}`,
  );
...
d('admin access & notes (live DB)', () => {
  ...
  it('lets an admin read customer profiles + audit; denies anon', async () => {
    const admView = await asAdmin(
      admin,
      (tx) => tx`select id from public.profiles where id = ${buyer}`,
    );
    expect(admView.length).toBe(1);
    const anonView = await asAnon(
      (tx) => tx`select id from public.profiles where id = ${buyer}`,
    ).catch(() => []);
  ...
  it('admin can add a note; the customer cannot read admin notes; customer cannot add notes', async () => {
    ...
    const note = await asAdmin(
      admin,
      (tx) =>
        tx`select id from public.admin_add_note('listing', ${listing}::uuid, 'REVIEW', 'Checked the listing details.', null, null)`,
    );
    ...
    const sellerView = await asUser(
      seller,
      (tx) => tx`select id from public.admin_notes where entity_id = ${listing}`,
    );
    ...
    await expectError(
      () =>
        asUser(
          seller,
          (tx) =>
            tx`select id from public.admin_add_note('listing', ${listing}::uuid, 'REVIEW', 'nope', null, null)`,
        ),
    ...
    await expectError(
      () =>
        asAdmin(
          admin,
          (tx) =>
            tx`select id from public.admin_add_note('listing', ${listing}::uuid, 'REVIEW', 'x', null, null)`,
        ),
      /INVALID_BODY|check|violat/i,
    );
  });
});
...
d('customer restriction (live DB)', () => {
  ...
  it('restricts a customer (audit written), blocks new offers, then restores', async () => {
    ...
    await asAdmin(
      admin,
      (tx) => tx`select id from public.admin_restrict_customer(${buyer}::uuid, 'ACCOUNT_REVIEW')`,
    );
    const p = await asService(
      (tx: Sql) => tx`select restricted_at from public.profiles where id = ${buyer}`,
    );
    ...
    await expectError(
      () =>
        asAdmin(
          admin,
          (tx) =>
            tx`select id from public.admin_restrict_customer(${buyer}::uuid, 'ACCOUNT_REVIEW')`,
        ),
      /ALREADY_RESTRICTED/,
    );
    ...
    await asAdmin(
      admin,
      (tx) => tx`select id from public.admin_restore_customer(${buyer}::uuid, 'ISSUE_RESOLVED')`,
    );
    ...
    await expectError(
      () =>
        asUser(
          buyer,
          (tx) =>
            tx`select id from public.admin_restrict_customer(${seller}::uuid, 'ACCOUNT_REVIEW')`,
        ),
      /FORBIDDEN|permission denied/i,
    );
    await expectError(
...
d('admin listing / offer / transaction controls (live DB)', () => {
  ...
  it('admin pauses and resumes a LIVE listing', async () => {
    ...
    await asAdmin(
      admin,
      (tx) =>
        tx`select id from public.admin_pause_listing(${listing}::uuid, 'INFORMATION_UNDER_REVIEW')`,
    );
    let s = await asService(
      (tx: Sql) => tx`select state from public.listings where id = ${listing}`,
    );
    expect((s[0] as { state: string }).state).toBe('PAUSED');
    expect(await auditCount('ADMIN_LISTING_PAUSED', listing)).toBe(1);
    await asAdmin(
      admin,
      (tx) => tx`select id from public.admin_resume_listing(${listing}::uuid, 'ISSUE_RESOLVED')`,
    );
    ...
    await expectError(
      () =>
        asAdmin(
          admin,
          (tx) =>
            tx`select id from public.admin_pause_listing(${draft}::uuid, 'OPERATIONAL_SAFETY')`,
        ),
      /NOT_LIVE/,
    );
  });
  ...
  it('admin closes an invalid offer thread without mutating proposals', async () => {
    ...
    const before = await asService(
      (tx: Sql) =>
        tx`select amount_aed::int as a from public.offer_proposals where thread_id = ${thread} order by created_at limit 1`,
    );
    await asAdmin(
      admin,
      (tx) =>
        tx`select id from public.admin_close_offer_thread(${thread}::uuid, 'OPERATIONAL_DATA_REPAIR')`,
    );
    const st = await asService(
      (tx: Sql) => tx`select status from public.offer_threads where id = ${thread}`,
    );
  ...
  it('admin cannot mutate an immutable proposal amount or transaction accepted amount', async () => {
    ...
    const tx = await callTx(
      buyer,
      (t) => t`select * from public.ensure_transaction(${thread}::uuid)`,
    );
    await expectError(
      () =>
        asAdmin(
          admin,
          (t) => t`update public.offer_proposals set amount_aed = 1 where thread_id = ${thread}`,
        ),
    ...
    await expectError(
      () =>
        asAdmin(
          admin,
          (t) => t`update public.transactions set accepted_amount_aed = 1 where id = ${tx.id}`,
        ),
      /immutable|permission denied/i,
    );
  });
  ...
  it('admin pauses transaction progression (blocks customer tasks), resumes, then marks failed', async () => {
    ...
    await asAdmin(
      admin,
      (t) =>
        t`select id from public.admin_pause_transaction(${id}::uuid, 'INFORMATION_UNDER_REVIEW')`,
    );
    ...
    const v = await asService(
      (t: Sql) => t`select version from public.transactions where id = ${id}`,
    );
    await expectError(
      () =>
        callTx(
          buyer,
          (t) =>
            t`select * from public.tx_complete_task(${id}::uuid, 'BUYER_CONFIRM_DETAILS', ${(v[0] as { version: number }).version})`,
        ),
      /PROGRESSION_PAUSED/,
    );
    ...
    await asAdmin(
      admin,
      (t) => t`select id from public.admin_resume_transaction(${id}::uuid, 'ISSUE_RESOLVED')`,
    );
    await asAdmin(
      admin,
      (t) =>
        t`select id from public.admin_mark_transaction_failed(${id}::uuid, 'REPEATED_SYSTEM_FAILURE', 'SYSTEM')`,
    );
    const st = await asService(
      (t: Sql) => t`select status from public.transactions where id = ${id}`,
    );
  ...
  it('admin document-access records the phased audit lifecycle; customer cannot; admin cannot edit audit history', async () => {
    ...
    await asAdmin(
      admin,
      (t) =>
        t`select public.admin_record_document_access('listing', ${listing}::uuid, 'OWNERSHIP', 'VERIFICATION_REVIEW', 'REQUESTED')`,
    );
    await asAdmin(
      admin,
      (t) =>
        t`select public.admin_record_document_access('listing', ${listing}::uuid, 'OWNERSHIP', 'VERIFICATION_REVIEW', 'GRANTED')`,
    );
    // A failed mint records a FAILED result — never a false "accessed" event.
    await asAdmin(
      admin,
      (t) =>
        t`select public.admin_record_document_access('listing', ${listing}::uuid, 'OWNERSHIP', 'VERIFICATION_REVIEW', 'FAILED')`,
    );
    ...
    await expectError(
      () =>
        asAdmin(
          admin,
          (t) => t`update public.audit_events set action = 'TAMPERED' where entity_id = ${listing}`,
        ),
      /permission denied/i,
    );
  });
...
async function createDraft(ownerId: string): Promise<string> {
  return asService(async (tx: Sql) => {
    const [l] =
      await tx`insert into public.listings (id, owner_id, title, state) values (gen_random_uuid(), ${ownerId}, 'Draft', 'DRAFT') returning id`;
