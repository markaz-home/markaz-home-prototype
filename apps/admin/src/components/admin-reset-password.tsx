...
export function AdminResetPassword() {
  ...
  if (!email) {
    return (
      <AuthCard title={t('resetTitle')} description={t('errSessionMissing')}>
        <Button asChild className="w-full"><Link href="/forgot-password">{t('forgotTitle')}</Link></Button>
      </AuthCard>
    );
  }
  ...
  async function onSubmit(data: ResetPasswordInput) {
    setError(null);
    if (!/^\d{6}$/.test(code)) { setError(t('errInvalidCode')); return; }
    const verify = await supabase.auth.verifyOtp({ email, token: code, type: 'recovery' });
    if (verify.error) { setError(t(AUTH_ERROR_KEYS[mapAuthError(verify.error)])); return; }
    const update = await supabase.auth.updateUser({ password: data.password });
    if (update.error) { setError(t(AUTH_ERROR_KEYS[mapAuthError(update.error)])); return; }
    ...
    <AuthCard title={t('resetTitle')} description={t('resetSubtitle', { email: maskEmail(email) })}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <FormField id="code" label={t('codeLabel')} required>
          <Input id="code" inputMode="numeric" autoComplete="one-time-code" maxLength={6} dir="ltr" placeholder="••••••" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))} className="text-center text-lg tracking-[0.4em]" />
        </FormField>
