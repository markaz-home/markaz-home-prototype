...
export function withParams(base: Record<string, string | number | undefined>, patch: Record<string, string | number | undefined>): string {
...
export function ListPagination({
  ...
  return (
    <nav className="flex items-center justify-between border-t pt-3 text-sm" aria-label="pagination">
      <span className="text-muted-foreground">{labels.range}</span>
      <div className="flex gap-2">
        <PagerLink disabled={!hasPrev} href={`${pathname}${withParams(params, { offset: Math.max(0, offset - limit) })}`}>
          <ChevronLeft className="h-4 w-4 rtl:rotate-180" aria-hidden /> {labels.prev}
        </PagerLink>
        <PagerLink disabled={!hasNext} href={`${pathname}${withParams(params, { offset: offset + limit })}`}>
...
function PagerLink({ disabled, href, children }: { disabled: boolean; href: string; children: React.ReactNode }) {
  if (disabled) {
    ...
    return (
      <button type="button" disabled className="inline-flex cursor-not-allowed items-center gap-1 rounded-md border px-3 py-1.5 text-muted-foreground opacity-50">
        {children}
      </button>
    );
  }
  return <Link href={href} className="inline-flex items-center gap-1 rounded-md border px-3 py-1.5 hover:bg-muted">{children}</Link>;
}
