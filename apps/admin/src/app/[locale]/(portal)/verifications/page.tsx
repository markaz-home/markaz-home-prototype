...
const LIMIT = 25;
type Row = Awaited<ReturnType<Awaited<ReturnType<typeof getServerApi>>['admin']['verifications']['list']>>[number];
...
export default async function VerificationsPage({
  ...
  const columns: Column<Row>[] = [
    { id: 'kind', header: t('verifications.col.kind'), priority: 'primary', cell: (r) => t.has(`verifications.kind.${r.kind}`) ? t(`verifications.kind.${r.kind}`) : r.kind },
    { id: 'status', header: t('verifications.col.status'), priority: 'secondary', cell: (r) => {
      const s = verificationStatusLabel(r.status);
      return (
        <span className="inline-flex items-center gap-2">
          <StatusBadge tone={s.tone} label={t.has(s.key) ? t(s.key) : r.status} />
          {r.superseded ? <span className="text-xs text-muted-foreground">{t('verifications.superseded')}</span> : null}
        </span>
      );
    } },
    { id: 'created', header: t('verifications.col.created'), priority: 'low', cell: (r) => formatWhen(r.createdAt) },
  ];
  ...
  return (
    <PageShell maxWidth={1600}>
      ...
      {rows.length === 0 ? (
        ...
        <>
          <DataTable columns={columns} rows={rows} rowKey={(r) => r.id} rowHref={(r) => `/verifications/${r.id}`} caption={t('verifications.title')} />
          <div className="mt-4">
            <ListPagination
