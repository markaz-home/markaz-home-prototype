...
export default async function CustomerDetailPage({ params }: { params: Promise<{ locale: string; customerId: string }> }) {
  ...
  let c: Awaited<ReturnType<Awaited<ReturnType<typeof getServerApi>>['admin']['customers']['get']>> | null = null;
  try {
    c = await (await getServerApi()).admin.customers.get({ id: customerId });
  } catch {
    c = null;
  }
  if (!c) return <PageShell maxWidth={560}><EmptyState title={t('customers.unavailable')} /></PageShell>;
  ...
  return (
    <PageShell maxWidth={1440}>
      <PageHeader
        ...
        actions={c.status === 'ACTIONS_RESTRICTED' ? <RestoreCustomerAction customerId={c.id} /> : <RestrictCustomerAction customerId={c.id} />}
      />
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <DataSection title={t('customers.title')}>
            <FieldGrid>
              <Field label={t('customers.col.status')} value={<StatusBadge tone={status.tone} label={t(status.key)} />} />
              <Field label={t('customers.identity')} value={c.identityStatus} />
              <Field label={t('customers.onboarded')} value={c.onboarded ? t('yes') : t('customers.notOnboarded')} />
          ...
          {c.status === 'ACTIONS_RESTRICTED' ? (
            <DataSection title={t('customers.status.restricted')} visibility="private">
              <FieldGrid>
                <Field label={t('customers.restrictionReason')} value={c.restrictionReason ? t(`customer.restrict.reason.${c.restrictionReason}`) : '—'} />
