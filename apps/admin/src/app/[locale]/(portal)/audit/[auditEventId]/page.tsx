...
export default async function AuditDetailPage({
  ...
  params: Promise<{ locale: string; auditEventId: string }>;
}) {
  ...
  let e: Awaited<
    ReturnType<Awaited<ReturnType<typeof getServerApi>>['admin']['audit']['get']>
  > | null = null;
  try {
  ...
  if (!e)
    return (
      <PageShell maxWidth={560}>
        <EmptyState title={t('audit.unavailable')} />
      </PageShell>
    );
      ...
      <DataSection title={t('audit.title')}>
        <FieldGrid>
          <Field
            ...
            value={
              <StatusBadge
                tone={actorTypeTone(e.actorType)}
                label={t(`audit.actor.${e.actorType}`)}
              />
            }
          />
          <Field
