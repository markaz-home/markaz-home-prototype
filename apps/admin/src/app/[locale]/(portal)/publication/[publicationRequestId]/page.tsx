...
import { ApprovePublicationAction, ReturnPublicationAction } from '@/components/admin/entity-actions';
import { publicationStatusLabel, formatAed, formatWhen } from '@/components/admin/labels';

export default async function PublicationDetailPage({ params }: { params: Promise<{ locale: string; publicationRequestId: string }> }) {
  const { locale, publicationRequestId } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('admin');

  let r: Awaited<ReturnType<Awaited<ReturnType<typeof getServerApi>>['admin']['publication']['get']>> | null = null;
  try {
    r = await (await getServerApi()).admin.publication.get({ id: publicationRequestId });
  } catch {
    r = null;
  }
  if (!r) return <PageShell maxWidth={560}><EmptyState title={t('publication.unavailable')} /></PageShell>;
  ...
  return (
    <PageShell maxWidth={1480}>
      ...
      />
      {!isPending ? <div className="mb-4"><StatusBadge tone={status.tone} label={t('publication.conflict')} /></div> : null}
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <DataSection title={t('publication.title')}>
            <FieldGrid>
              <Field label={t('publication.col.status')} value={<StatusBadge tone={status.tone} label={t(status.key)} />} />
              <Field label={t('publication.col.submitted')} value={formatWhen(r.submittedAt)} />
              {r.listing ? <Field label={t('listing.askingPrice')} value={formatAed(r.listing.askingPriceAed)} ltr /> : null}
            </FieldGrid>
          </DataSection>
            ...
            <ul className="space-y-2">
              {checks.map((c) => (
                <li key={c.key} className="flex items-center justify-between gap-2 text-sm">
                  ...
                  {c.pass ? (
                    <span className="inline-flex items-center gap-1 text-emerald-700"><CheckCircle2 className="h-4 w-4" aria-hidden />{t('publication.checklist.pass')}</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-red-700"><XCircle className="h-4 w-4" aria-hidden />{t('publication.checklist.fail')}</span>
                  )}
                </li>
              ))}
            </ul>
