...
import {
  Users, Ban, Building2, PauseCircle, CheckCircle2, Tag, Receipt, XCircle, FileCheck2, AlertTriangle,
} from 'lucide-react';
...
type Metrics = Awaited<ReturnType<Awaited<ReturnType<typeof getServerApi>>['admin']['overview']['metrics']>>;
...
export default async function OverviewPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const cards: { icon: typeof Users; key: keyof Metrics; href: string }[] = [
    ...
    { icon: CheckCircle2, key: 'completedTransactions', href: '/transactions?status=COMPLETED_DEMO' },
  ...
  return (
    <PageShell maxWidth={1600}>
      ...
      {failed ? <Alert variant="warning" className="mb-6">{t('overview.partialError')}</Alert> : null}

      <section aria-label={t('overview.queue.title')} className="mb-8">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">{t('overview.queue.title')}</h2>
        ...
        {attention.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {attention.map((q) => (
              <Link key={q.key} href={q.href} className="rounded-lg border bg-amber-50 p-4 transition-colors hover:bg-amber-100">
          ...
          <p className="rounded-lg border bg-muted/40 p-4 text-sm text-muted-foreground">{t('overview.queue.empty')}</p>
        )}
      </section>
          ...
          <Link key={key} href={href}>
            <Card className="h-full transition-colors hover:bg-muted/40">
              <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground">{t(`overview.metric.${key}`)}</CardTitle>
