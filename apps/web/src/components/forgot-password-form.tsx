...
export function ForgotPasswordForm() {
  ...
  async function onSubmit(data: ForgotPasswordInput) {
    ...
    // Only surface rate-limit / provider issues. Never reveal account existence.
    if (err) {
      const key = mapAuthError(err);
      if (key === 'rate_limited' || key === 'provider_unavailable') {
        setError(t(AUTH_ERROR_KEYS[key]));
        return;
      }
    }
  ...
  if (sent) {
    return (
      <AuthCard title={t('forgotTitle')}>
        ...
        <Button className="w-full" onClick={() => router.push(`/reset-password?email=${encodeURIComponent(sentEmail)}`)}>
          {t('enterRecoveryCode')}
        </Button>
