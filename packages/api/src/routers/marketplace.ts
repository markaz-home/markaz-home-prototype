...
import { z } from 'zod';
import { listings, savedProperties, auditEvents, marketplaceListings as mv, type Tx } from '@markaz/db';
...
function buildConditions(input: MarketplaceQuery): SQL[] {
  ...
  if (input.q) {
    const like = `%${input.q}%`;
    const m = or(ilike(mv.community, like), ilike(mv.emirate, like), ilike(mv.buildingOrProject, like), ilike(mv.propertyType, like));
    if (m) c.push(m);
  }
  return c;
}
...
async function loadLiveListing(tx: Tx, publicId: string): Promise<{ id: string; ownerId: string } | null> {
  const [row] = await tx
...
export const marketplaceRouter = router({
  ...
  search: publicTxProcedure.input(marketplaceQuerySchema).query(async ({ ctx, input }) => {
    ...
    const totalRows = await ctx.tx.select({ total: sql<number>`count(*)::int` }).from(mv).where(where);
  ...
  getByPublicId: publicTxProcedure.input(z.object({ publicId: z.string().max(40) })).query(async ({ ctx, input }) => {
  ...
  getFilterOptions: publicTxProcedure.query(async ({ ctx }) => {
    const rows = await ctx.tx.select({ emirate: mv.emirate, community: mv.community, type: mv.propertyType }).from(mv);
  ...
  saved: router({
    ...
    save: customerProcedure.input(z.object({ publicId: z.string().max(40) })).mutation(async ({ ctx, input }) => {
      const listing = await loadLiveListing(ctx.tx, input.publicId);
      if (!listing) throw new TRPCError({ code: 'NOT_FOUND', message: 'This property is no longer available.' });
      if (listing.ownerId === ctx.user.id) throw new TRPCError({ code: 'BAD_REQUEST', message: 'You cannot save your own listing.' });
      await ctx.tx.insert(savedProperties).values({ customerId: ctx.user.id, listingId: listing.id }).onConflictDoNothing();
      await ctx.tx.insert(auditEvents).values({ actorId: ctx.user.id, action: 'PROPERTY_SAVED', entityType: 'listing', entityId: listing.id, metadata: {} });
      return { saved: true as const };
    }),
    remove: customerProcedure.input(z.object({ publicId: z.string().max(40) })).mutation(async ({ ctx, input }) => {
      const [l] = await ctx.tx.select({ id: listings.id }).from(listings).where(eq(listings.publicId, input.publicId)).limit(1);
      if (l) {
        await ctx.tx.delete(savedProperties).where(and(eq(savedProperties.customerId, ctx.user.id), eq(savedProperties.listingId, l.id)));
        await ctx.tx.insert(auditEvents).values({ actorId: ctx.user.id, action: 'PROPERTY_SAVE_REMOVED', entityType: 'listing', entityId: l.id, metadata: {} });
      }
      return { saved: false as const };
    }),
    removeById: customerProcedure.input(z.object({ savedId: z.string().uuid() })).mutation(async ({ ctx, input }) => {
    ...
    isSaved: customerProcedure.input(z.object({ publicId: z.string().max(40) })).query(async ({ ctx, input }) => {
      ...
      const [s] = await ctx.tx
        ...
        .where(and(eq(savedProperties.customerId, ctx.user.id), eq(savedProperties.listingId, l.id)))
        .limit(1);
      return { saved: !!s };
    }),
    ...
    list: customerProcedure.query(async ({ ctx }) => {
      const saves = await ctx.tx
        .select({ savedId: savedProperties.id, listingId: savedProperties.listingId, savedAt: savedProperties.createdAt })
        .from(savedProperties)
      ...
      for (const s of saves) {
        const [row] = await ctx.tx
          .select()
          .from(mv)
          .where(eq(mv.publicId, sql`(select public_id from public.listings where id = ${s.listingId})`))
          .limit(1);
        if (row) items.push({ kind: 'available', savedId: s.savedId, savedAt: s.savedAt.toISOString(), card: toPublicCard(toRow(row)) });
        else items.push({ kind: 'unavailable', savedId: s.savedId, savedAt: s.savedAt.toISOString() });
      }
      return items;
    }),
  }),
